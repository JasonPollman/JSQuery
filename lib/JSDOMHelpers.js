(function () {
    ...
    var JSDOMConstants = require('./JSDOMConstants'),
        ...
        POLL  = JSDOMConstants.POLL_SELECTOR_STATS,
        stats = require('./Stat'),

        /**
         * The last time the index was updated.
         * If lastIndexUpdate === index#update, then cached selector results can be returned.
         * @type {Number}
         */
        lastIndexUpdate = -1,
    ...
    function filterByNth (elems, nth) {
        if(POLL) stats.current.filtering.nth++;
    ...
    function filterByDescendants (elements, parent, filtered) {
        if(POLL) stats.current.filtering.descendants++;
    ...
    function filterByAttributes (elements, attributes) {
        if(POLL) stats.current.filtering.attributes++;
    ...
    function getChildrenOf (e) {
        if(e && typeof e === 'object' && typeof e.children === 'object') return e.children;
        return [];
    }
    ...
    function filterByParsedSelector (index, parsed, parent) {
        if(POLL) stats.current.filtering.selector++;
    ...
    function findElementsWithSelector(index, selector) {
        ...
        var tokens = selector.split(regexps.selector)._.without('', ' '),
            ...
            uid = index._.uniqueId(),
            md5 = crypto.createHash('md5').update(tokens.join('')).digest('hex');

        if(POLL) stats.current.md5 = md5;
        if(!cache[uid]) cache[uid] = {};

        if(index.updated === lastIndexUpdate) {
            if(cache[uid][md5]) {
                if(POLL) {
                    end = process.hrtime(start);
                    stats.current.cached = true;
                    stats.current.time   = ((end[0] * 1e9 + end[1]) * 1e-6);
                }
                return cache[uid][md5];
            }
        }
        else {
            lastIndexUpdate = index.updated;
        }
        ...
        // Remove the LFU (least frequently used) cache line item...
        if(cache[uid]._.size() > JSDOMConstants.MAX_SELECTOR_CACHE) {
            var lfu = cache[uid]._.keyOfMin(function (line) {
                return line.used;
            });
            cache[uid][lfu] = null;
            delete cache[uid][lfu];
        }
        return res;
    }

    findElementsWithSelector.killCache = function () {
